          ...
          <div className="space-y-3.5">
            ...
            <div className="grid grid-cols-3 gap-2">
              {(["solid", "gradient", "image"] as const).map((option) => {
                const selected = chatBackground.type === option;
                const label = option === "solid" ? "Color" : option === "gradient" ? "Gradient" : "Image";
                return (
                  <button
                    key={option}
                    type="button"
                    onClick={() => setChatBackground({ type: option })}
            ...
            {chatBackground.type === "solid" && (
              <div className="space-y-2">
                <div className="flex flex-wrap gap-2">
                  {solidPresets.map((preset) => (
                    <button
                      key={preset}
                      type="button"
                      onClick={() => setChatBackground({ type: "solid", color: preset })}
                      className={`h-8 w-8 rounded-full border-2 transition ${chatBackground.color === preset ? "border-emerald-400" : "border-theme"}`}
                ...
                <label className="flex items-center gap-2 text-[10px] text-secondary">
                  ...
                  <input
                    type="color"
                    value={chatBackground.color || "#0f172a"}
                    onChange={(event) => setChatBackground({ type: "solid", color: event.target.value })}
            ...
            {chatBackground.type === "gradient" && (
              <div className="space-y-2">
                <div className="grid grid-cols-2 gap-2">
                  {gradientPresets.map((preset) => (
                    <button
                      key={preset}
                      type="button"
                      onClick={() => setChatBackground({ type: "gradient", gradient: preset })}
                      className={`h-12 rounded-xl border transition ${chatBackground.gradient === preset ? "border-emerald-400" : "border-theme"}`}
                ...
                <input
                  type="text"
                  value={chatBackground.gradient}
                  onChange={(event) => setChatBackground({ type: "gradient", gradient: event.target.value })}
                ...
                <input
                  ...
                  onChange={(event) => {
                    ...
                    reader.onload = () => {
                      ...
                      setChatBackground({ type: "image", imageUrl: result });
                    };
                    reader.readAsDataURL(file);
                  }}
