...
import { useChatStore, checkApiReachable } from "./store.js";
...
export default function App() {
  ...
  useEffect(() => {
    if (token) {
      ...
      // Hide sidebar automatically after login on mobile-sized screens.
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // If reachable and we have a token, attempt to fetch user and conversations automatically
      if (reachable && token) {
        setLoading(true);
        try {
          await fetchUser();
      ...
      <div className={`w-screen h-screen ${themeClass} bg-app flex flex-col items-center justify-center text-theme font-medium`}>
        <div className="flex flex-col items-center">
          ...
          {serverReachable === false && (
            <span className="text-xs text-muted mt-2">Cannot reach API — check `VITE_MOBILE_API_BASE_URL`</span>
          )}
        </div>
      </div>
    );
  ...
  if (token && user) {
    return (
      <div className={`w-screen h-screen ${themeClass} bg-app flex overflow-hidden`}>
        <Sidebar
          onOpenConverter={() => setActiveView("converter")}
