    ...
    const doc = new Document({
        sections: [{
            ...
            children: paragraphs.map((paragraph, index) => new Paragraph({
                children: [new TextRun({ text: paragraph, size: 12, color: "000000" })],
                spacing: { after: 120 },
            })),
        }],
    });
...
export async function createPdfBlob(text: string, accentColor: string, applyTheme: boolean) {
    ...
    const drawLine = (line: string, size: number, offsetGap = false) => {
    ...
    contentLines.forEach((line, index) => {
        ...
        drawLine(line, 12, isParagraphStart);
    });
...
export async function createXlsxBlob(text: string) {
    ...
    const worksheet = XLSX.utils.aoa_to_sheet(rows.length > 0 ? rows : [["No extracted content"]]);
...
export async function createPptxBlob(text: string, accentColor: string, applyTheme: boolean) {
    ...
    slide.addText(paragraphs.slice(0, 6).join("\n\n"), {
        x: 0.5,
        y: 0.5,
        w: 9.0,
        h: 5.0,
...
function isSameFormatOutput(file: File, target: string) {
    const extension = getFileExtension(file.name);
    const normalizedTarget = target === "jpg" ? "jpg" : target === "jpeg" ? "jpg" : target;
    const normalizedExtension = extension === "jpeg" ? "jpg" : extension;

    if (normalizedTarget === normalizedExtension) return true;
    if (normalizedTarget === "pdf" && normalizedExtension === "pdf") return true;
    if (normalizedTarget === "docx" && normalizedExtension === "docx") return true;
    if (normalizedTarget === "pptx" && normalizedExtension === "pptx") return true;
    if (normalizedTarget === "xlsx" && normalizedExtension === "xlsx") return true;
    if (normalizedTarget === "txt" && normalizedExtension === "txt") return true;
    if (normalizedTarget === "md" && normalizedExtension === "md") return true;
    if (normalizedTarget === "svg" && normalizedExtension === "svg") return true;
    return false;
}

export async function createOutputBlob(file: File, target: string, accentColor: string, applyTheme: boolean) {
    if (isSameFormatOutput(file, target)) {
        return file;
    }
    ...
    switch (target) {
        ...
        case "html":
            return createTextBlob(text.replace(/\n/g, "<br />"), "html");
        case "png":
        case "jpg":
        case "webp":
