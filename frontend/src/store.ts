...
export function getApiBaseUrl(): string {
  ...
  const envUrl = env.VITE_API_BASE_URL || env.VITE_MOBILE_API_BASE_URL || env.VITE_APP_API_BASE_URL;
...
export function getWebFallbackUrl(): string | null {
  const env = (import.meta as any).env as Record<string, any>;
  const url = env.VITE_MOBILE_WEB_APP_URL || env.VITE_WEB_APP_URL || env.VITE_APP_WEB_URL;
...
export function isNativeMobileWebView(): boolean {
...
const LOCAL_GUEST_CONVERSATIONS_KEY = "sucharai_guest_conversations";
const LOCAL_GUEST_MESSAGES_KEY = "sucharai_guest_messages";
const LOCAL_GUEST_DOCUMENTS_KEY = "sucharai_guest_documents";
...
export const useChatStore = create<ChatState>((set, get) => {
  ...
  return {
    ...
    fetchConversations: async () => {
      ...
      if (!currentUid) {
        const list = readLocalStorageArray<Conversation>(LOCAL_GUEST_CONVERSATIONS_KEY)
    ...
    createConversation: async (title) => {
      ...
      if (!currentUid) {
        ...
        const nextConversations = [newConv, ...readLocalStorageArray<Conversation>(LOCAL_GUEST_CONVERSATIONS_KEY).filter((c) => c.id !== newConv.id)];
        writeLocalStorageArray(LOCAL_GUEST_CONVERSATIONS_KEY, nextConversations);
        set((state) => ({ conversations: [newConv, ...state.conversations.filter((c) => c.id !== newConv.id)] }));
        return newConv.id;
      }
    ...
    renameConversation: async (id, title) => {
      const currentUid = auth.currentUser?.uid || get().token;
      if (!currentUid) {
        const nextConversations = readLocalStorageArray<Conversation>(LOCAL_GUEST_CONVERSATIONS_KEY).map((c) => (c.id === id ? { ...c, title, updatedAt: new Date().toISOString() } : c));
        writeLocalStorageArray(LOCAL_GUEST_CONVERSATIONS_KEY, nextConversations);
        set((state) => ({
    ...
    archiveConversation: async (id, isArchived) => {
      const currentUid = auth.currentUser?.uid || get().token;
      if (!currentUid) {
        const nextConversations = readLocalStorageArray<Conversation>(LOCAL_GUEST_CONVERSATIONS_KEY).map((c) => (c.id === id ? { ...c, isArchived, updatedAt: new Date().toISOString() } : c));
        writeLocalStorageArray(LOCAL_GUEST_CONVERSATIONS_KEY, nextConversations);
        set((state) => ({
    ...
    deleteConversation: async (id) => {
      const currentUid = auth.currentUser?.uid || get().token;
      if (!currentUid) {
        const nextConversations = readLocalStorageArray<Conversation>(LOCAL_GUEST_CONVERSATIONS_KEY).filter((c) => c.id !== id);
        writeLocalStorageArray(LOCAL_GUEST_CONVERSATIONS_KEY, nextConversations);
        set((state) => ({
    ...
    deleteConversations: async (ids) => {
      ...
      const currentUid = auth.currentUser?.uid || get().token;
      if (!currentUid) {
        const nextConversations = readLocalStorageArray<Conversation>(LOCAL_GUEST_CONVERSATIONS_KEY).filter((c) => !normalizedIds.includes(c.id.toString()));
        writeLocalStorageArray(LOCAL_GUEST_CONVERSATIONS_KEY, nextConversations);
        set((state) => {
    ...
    fetchMessages: async (conversationId) => {
      const currentUid = auth.currentUser?.uid || get().token;
      if (!currentUid) {
        const list = readLocalStorageArray<Message>(LOCAL_GUEST_MESSAGES_KEY)
    ...
    fetchDocuments: async (conversationId) => {
      ...
      if (!currentUid) {
        const list = readLocalStorageArray<DocumentRecord>(LOCAL_GUEST_DOCUMENTS_KEY)
          .filter((doc: any) => doc.conversationId.toString() === conversationId.toString())
          .sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
        set({ documents: list });
        return;
      }
    ...
    uploadFile: async (conversationId, file) => {
      ...
      try {
        ...
        const conversionPromise = api.post(`/api/fast-convert`, formData, {
        ...
        const [conversionResult, uploadResult] = await Promise.allSettled([conversionPromise, uploadPromise]);
        const storageUrl = uploadResult.status === "fulfilled" ? uploadResult.value : "";
        const docData = conversionResult.status === "fulfilled" ? conversionResult.value : null;
        const extractedText = docData?.normalizedText || docData?.text || "";

        if (!extractedText && conversionResult.status === "fulfilled") {
    ...
    sendChatMessage: async (conversationId, content, onChunk) => {
      ...
      set((state) => {
        const nextMessages = [...state.messages, localUserMsg, localAssistantMsg];
        if (!auth.currentUser?.uid && !get().token) {
          writeLocalStorageArray(LOCAL_GUEST_MESSAGES_KEY, nextMessages);
        }
      ...
      try {
        ...
        const candidateUrls = [
          `${apiBaseUrl}/api/conversations/${conversationId}/chat`,
          `http://127.0.0.1:3000/api/conversations/${conversationId}/chat`,
          `http://localhost:3000/api/conversations/${conversationId}/chat`,
        ];
        ...
        for (const url of candidateUrls) {
          try {
            ...
            lastError = new Error(`Chat request failed with ${response.status} for ${url}`);
          } catch (err) {
            lastError = err;
          }
        }
        ...
        console.log("Chat request sent to API:", response);
