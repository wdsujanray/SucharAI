...
async function detectObjectsFromImage(file: any) {
  ...
  const promptPart = {
    text: `Analyze this image for object detection. Detect all major, distinct objects in the image. For each object detected, provide: 1. A label/name of the object. 2. The normalized bounding box [ymin, xmin, ymax, xmax] as integers between 0 and 1000 representing the percentage of image dimensions. 3. Your confidence score between 0.00 and 1.00. Return the result as a strictly valid JSON array of objects, with no markdown code fences or backticks (no \`\`\`json, no preamble, no explanation). The JSON array must have elements matching this schema: [{"box_2d": [ymin, xmin, ymax, xmax], "label": "string", "confidence": number}]`,
  };
...
const authenticateToken = (req: AuthenticatedRequest, res: express.Response, next: express.NextFunction) => {
  ...
  jwt.verify(token, JWT_SECRET, (err, decoded: any) => {
    if (err || !decoded || !decoded.sub) {
      req.user = {
        id: token.length > 5 ? token : "fb-user",
